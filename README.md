...
```bash
bash <(wget -qO- https://namingo.org/install-fossbilling-epp.sh) namingo
```

Replace `namingo` with the registry name. Run without parameters to see all supported registries:

```bash
bash <(wget -qO- https://namingo.org/install-fossbilling-epp.sh)
```

The installer automatically detects FOSSBilling under `/var/www`. You may also specify the path explicitly:

```bash
bash <(wget -qO- https://namingo.org/install-fossbilling-epp.sh) namingo /var/www/fossbilling
```
...
3. Within FOSSBilling, go to **System -> Domain Registration -> Registrars -> New Domain Registrar** and activate the new domain registrar.

4. Go to the **Registrars** tab in the admin panel and select your EPP module. Enter your registry connection details, including the EPP host, port, login credentials, and the full filesystem paths to your client TLS certificate (`cert.pem`) and private key (`key.pem`).
